...
,Editor({
    ...
    set: function (data) {
        ...
        data.plus=data.axisLocation_plus-dplus
        if (data.plus < 0){
            data.plus=data.plus+100
            data.pk=data.axisLocation_pk - dpk - 1
        }
        else{
            data.pk=data.axisLocation_pk - dpk
        }
        if (data.pk<0){
            data.pk=data.pk+10
            data.km=data.axisLocation_km - dkm - 1
        }
        else {
            data.km=data.axisLocation_km - dkm
        }

        data.endLocation_plus=data.axisLocation_plus+dplus
        if (data.endLocation_plus>=${Location.METER_IN_PK}){
            data.endLocation_plus=data.endLocation_plus-100
            data.endLocation_pk=data.axisLocation_pk + dpk + 1
        }
        else{
            data.endLocation_pk=data.axisLocation_pk + dpk
        }
        if (data.endLocation_pk>9){
            data.endLocation_pk=data.endLocation_pk-10
            data.endLocation_km=data.axisLocation_km + dkm + 1
        }
        else {
            data.endLocation_km=data.axisLocation_km + dkm
        }
